...
open class OAuthSwiftCredential: NSObject, NSCoding {
    ...
    open class func generateNonce() -> String {
        ...
        while let range = uuidString.range(of: "-") {
            uuidString.replaceSubrange(range, with: "k")
        }
        let thing = uuidString.substring(to: 32).urlEncodedString
        assert(thing.characters.count == 32)
        return thing
    }
